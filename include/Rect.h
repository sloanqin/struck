...
/*友元函数需要在类外重新声明和定义，类内的声明只是说明访问权限，并不是通常意义上的声明*/
/*操作符重载的定义*/
template <typename T>
...
float Rect<T>::Overlap(const Rect<T2>& rOther) const
{
	...
	float areaInt = (x1-x0)*(y1-y0);//重合的面积
	return areaInt/((float)Area()+(float)rOther.Area()-areaInt);//重合占比，重合面积/（a1 U a2）
}
