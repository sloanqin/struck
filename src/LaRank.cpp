...
double LaRank::Evaluate(const Eigen::VectorXd& x, const FloatRect& y) const//论文中公式10后半部分计算，即f=S(x,y)
{
	double f = 0.0;
	for (int i = 0; i < (int)m_svs.size(); ++i)
	{
		const SupportVector& sv = *m_svs[i];//遍历每一个支持向量
		f += sv.b*m_kernel.Eval(x, sv.x->x[sv.y]);//beta*高斯核,然后累加，得到score
	}
	return f;
}
...
{
	const FloatRect& centre(sample.GetRects()[0]);//上一帧目标框的矩形
	vector<VectorXd> fvs;
	//const_cast的作用是强制类型转换，去掉常量属性，所以现在m_features是可以编辑的了
	//看看Features这个类的特点先
	const_cast<Features&>(m_features).Eval(sample, fvs);//fvs 存放haar特征值
	results.resize(fvs.size());//把输出vector的大小调整到跟sample的rect的个数一样
	for (int i = 0; i < (int)fvs.size(); ++i)
	{
		// express y in coord frame of centre sample
		FloatRect y(sample.GetRects()[i]);
		y.Translate(-centre.XMin(), -centre.YMin());//每个rect的横纵坐标减去上一帧框的横纵坐标
		results[i] = Evaluate(fvs[i], y);//Evaluate就是F函数，输入每个rect的特征x和y，计算对应的分数score
	}
}
...
void LaRank::Update(const MultiSample& sample, int y)
{
	...
	FloatRect centre = rects[y];//对于真值rect，y=0，获得目标框
	for (int i = 0; i < (int)rects.size(); ++i)//把所有的样本框的位置 图片添加到新建的sp中
	{
		// express r in coord frame of centre sample
		FloatRect r = rects[i];
		r.Translate(-centre.XMin(), -centre.YMin());//r减去了真值框的x y
		sp->yv.push_back(r);//在支持模式中存储矩形框的位置变化关系
		if (!m_config.quietMode && m_config.debugMode)
		{
			// store a thumbnail for each sample
			Mat im(kTileSize, kTileSize, CV_8UC1);//创建一个30*30的图像
			IntRect rect = rects[i];
			cv::Rect roi(rect.XMin(), rect.YMin(), rect.Width(), rect.Height());
			cv::resize(sample.GetImage().GetImage(0)(roi), im, im.size());
			sp->images.push_back(im);//将样本的图片存储在sp中，大小都归一化到30*30
		}
	}
	// evaluate features for each sample
	sp->x.resize(rects.size());
	const_cast<Features&>(m_features).Eval(sample, sp->x);//提取特征，存储到sp中
	sp->y = y;
	sp->refCount = 0;
	m_sps.push_back(sp);//将刚创建的sp，添加到vector中

	ProcessNew((int)m_sps.size()-1);//使用刚创建的sp，执行ProcessNew
	BudgetMaintenance();
	
	for (int i = 0; i < 10; ++i)//次数关系，ProcessNew：Reprocess=1：10
	{
		Reprocess();
		BudgetMaintenance();
	}
...
pair<int, double> LaRank::MinGradient(int ind)//输入ind是vector sp的序号
{
	const SupportPattern* sp = m_sps[ind];
	pair<int, double> minGrad(-1, DBL_MAX);
	for (int i = 0; i < (int)sp->yv.size(); ++i)//寻找这个支持模式sp中gradient最小的y
	{
...
void LaRank::ProcessNew(int ind)//添加支持向量，调整beta值
{
	...
	pair<int, double> minGrad = MinGradient(ind);//对x，找使梯度最小的y
	int in = AddSupportVector(m_sps[ind], minGrad.first, minGrad.second);

	SMOStep(ip, in);//调整beta和gradient，论文中的algorithm 1
}
...
void LaRank::ProcessOld()
{
	...
	int ind = rand() % m_sps.size();//随机选取一个sp

	// find existing sv with largest grad and nonzero beta
	int ip = -1;
	double maxGrad = -DBL_MAX;
	for (int i = 0; i < (int)m_svs.size(); ++i)//遍历每一个支持向量
	{
		if (m_svs[i]->x != m_sps[ind]) continue;//找到与sp对应的sv

		const SupportVector* svi = m_svs[i];//copy这个sv
		if (svi->g > maxGrad && svi->b < m_C*(int)(svi->y == m_sps[ind]->y))//找gradient最大的sv，且要beta满足约束条件
		{
			ip = i;
			maxGrad = svi->g;
		}
	...
	for (int i = 0; i < (int)m_svs.size(); ++i)//遍历每一个支持向量
	{
		...
		if (m_svs[i]->y == minGrad.first)//如果y在支持向量vector中
		{
			in = i;
			break;
		}
	}
	if (in == -1)//如果对应的y不在所有sv中，添加新的sv
	{
		// add new sv
