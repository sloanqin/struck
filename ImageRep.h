...
private:
	std::vector<cv::Mat> m_images;//存储各个通道的图像，目前使用的是单通道，也就是转换成了gray image
	std::vector<cv::Mat> m_integralImages;//存储各个通道的积分图
	std::vector<cv::Mat> m_integralHistImages;
	int m_channels;
	IntRect m_rect;//给定一个矩形框，利用积分图就可以很方面得到矩形框的像素点的面积了
};
